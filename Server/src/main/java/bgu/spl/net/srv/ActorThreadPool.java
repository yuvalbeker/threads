...
public class ActorThreadPool {
    ...
    public void submit(Object act, Runnable r) {
        synchronized (act) {
            if (!playingNow.contains(act)) { //only one thread can do an action of one handler
                playingNow.add(act);
                execute(r, act); //run r and complee
            } else {
    ...
    private Queue<Runnable> pendingRunnablesOf(Object act) { //return the Queue of runnable acts of the handler if exists. create a new one if it doesn't.
    ...
    private void complete(Object act) {
        synchronized (act) {
            ...
            if (pending.isEmpty()) {
                ...
                execute(pending.poll(), act); //run the next action of the handler (client)
            }
        }
    }
